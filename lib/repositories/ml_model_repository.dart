...
class MLModelRepository {
  ...
  /// 特定のステータスのモデルを取得
  Stream<List<MLModel>> watchModelsByStatus(
      String userId, MLModelStatus status) {
  ...
  Future<void> updateStatus(
      String userId, String modelId, MLModelStatus newStatus) async {
    await _firestore
        .collection('users')
        .doc(userId)
        .collection('mlModels')
        ...
        .update({
          ...
          'deployedAt': newStatus == MLModelStatus.production
  ...
  Future<void> updateInferenceStats(
    ...
    // 新しい平均レイテンシを計算
    final newAvgLatency =
        (currentAvgLatency * currentTotal + latencyMs) / (currentTotal + 1);

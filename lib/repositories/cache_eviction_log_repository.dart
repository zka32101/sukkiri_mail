...
class CacheEvictionLogRepository {
  ...
  CacheEvictionLogRepository({FirebaseFirestore? db}) : _db = db ?? FirebaseFirestore.instance;

  CollectionReference<Map<String, dynamic>> get _col => _db.collection('cacheEvictionLogs');
  ...
  Future<int> totalFreedBytes(String userId) async {
    ...
    return snap.docs.fold<int>(
      0,
      (total, d) => total + ((d.data()['freedBytesEstimate'] as num?)?.toInt() ?? 0),
    );
  }

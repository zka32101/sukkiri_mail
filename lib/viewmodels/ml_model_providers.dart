...
final mlModelComparisonProvider =
  ...
  final prodModelAsync = ref.watch(productionCategorizationModelProvider(userId));
...
final mlModelUsageStatsProvider =
  ...
  final prodModels = models
      .where((m) => m.status == MLModelStatus.production)
      .toList();
  final stagingModels = models
      .where((m) => m.status == MLModelStatus.staging)
      .toList();
...
class ABTestEvaluation {
  ...
  final double accuracyImprovement; // パーセント（正の値=ステージング優秀）
